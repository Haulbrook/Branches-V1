...
const CONFIG = {
  ...
  ACTIVE_WORK_ORDERS_SHEET_ID: "13bnntgZiXdCA2KQvJXpIBA1rCBzx3ZmeKakzYFFg7QA", // Deep Roots Active Work Orders sheet
...
function doPost(e) {
  try {
    ...
    switch(functionName) {
      ...
      case 'getActiveWorkOrders':
        result = getActiveWorkOrders();
        break;
...
// ═══════════════════════════════════════════════════════════════════════
// 📊 DASHBOARD STATS & ACTIVE WORK ORDERS
// ═══════════════════════════════════════════════════════════════════════

/**
 * Get dashboard statistics for the hero stats cards
 * Returns: Plant Varieties count, Active Vehicles count, Active Jobs count
 * Note: Tools Checked Out is calculated on the frontend from localStorage
 * @returns {Object} Dashboard statistics
 */
function getDashboardStats() {
  Performance.start('getDashboardStats');
  ...
  try {
    ...
    // Get inventory count (plant varieties)
    try {
      const invSS = SpreadsheetApp.openById(CONFIG.INVENTORY_SHEET_ID);
      const invSheet = invSS.getSheetByName(CONFIG.INVENTORY_SHEET_NAME);
      const invData = invSheet.getDataRange().getValues();
      // Count non-empty rows (excluding header)
      stats.inventoryCount = invData.filter((row, i) => i > 0 && row[0]).length;
    } catch (e) {
      Logger.log('Error getting inventory count: ' + e.toString());
    }

    // Get fleet count (active vehicles)
    try {
      const truckSS = SpreadsheetApp.openById(CONFIG.TRUCK_SHEET_ID);
      const truckSheet = truckSS.getSheetByName(CONFIG.TRUCK_SHEET_NAME);
      const truckData = truckSheet.getDataRange().getValues();

      for (let i = 1; i < truckData.length; i++) {
        const truckName = truckData[i][0];
        const model = truckData[i][1];
        const status = String(truckData[i][4] || '').toLowerCase();

        if (!truckName) continue;

        if (status.includes('active')) {
          stats.fleetCount++;
          stats.activeVehicles.push({ name: truckName, model: model, status: 'Active' });
        } else {
          stats.inactiveVehicles.push({ name: truckName, model: model, status: status || 'Unknown' });
        }
      }
    } catch (e) {
      Logger.log('Error getting fleet count: ' + e.toString());
    }

    // Get active work orders count from Active Work Orders sheet
    try {
      const woSS = SpreadsheetApp.openById(CONFIG.ACTIVE_WORK_ORDERS_SHEET_ID);
      const woSheet = woSS.getSheetByName(CONFIG.WORK_ORDERS_SHEET_NAME);
      const woData = woSheet.getDataRange().getValues();

      // Count active jobs (non-empty rows excluding header)
      for (let i = 1; i < woData.length; i++) {
        const woNumber = woData[i][0];
        const status = String(woData[i][4] || '').toLowerCase(); // Job Status column E
        if (woNumber && !status.includes('complete') && !status.includes('archived')) {
          stats.jobsCount++;
        }
      }
    } catch (e) {
      Logger.log('Error getting work orders count: ' + e.toString());
    }

    Performance.end('getDashboardStats');
    ...
    Performance.end('getDashboardStats');
    return ErrorHandler.createErrorResponse(error, 'getDashboardStats');
  }
}

/**
 * Get all active work orders with full details
 * Columns: woNumber, Job Description, Customer Name, Job Type, Job Status, Job Address, Salesman, Vehicle, Trailer
 * @returns {Object} List of active work orders
 */
function getActiveWorkOrders() {
  Performance.start('getActiveWorkOrders');

  try {
    const ss = SpreadsheetApp.openById(CONFIG.ACTIVE_WORK_ORDERS_SHEET_ID);
    const woSheet = ss.getSheetByName(CONFIG.WORK_ORDERS_SHEET_NAME);
    const woData = woSheet.getDataRange().getValues();
    ...
    // Find column indices (flexible in case columns move)
    const colIndex = {
      woNumber: headers.indexOf('woNumber') !== -1 ? headers.indexOf('woNumber') : 0,
      jobDescription: 1,
      customerName: 2,
      jobType: 3,
      jobStatus: 4,
      jobAddress: 5,
      salesman: 6,
      vehicle: headers.indexOf('Vehicle') !== -1 ? headers.indexOf('Vehicle') : 7,
      trailer: headers.indexOf('Trailer') !== -1 ? headers.indexOf('Trailer') : 8
    };

    for (let i = 1; i < woData.length; i++) {
      const row = woData[i];
      const woNumber = row[colIndex.woNumber];

      if (!woNumber) continue;

      const status = String(row[colIndex.jobStatus] || '').toLowerCase();

      // Skip completed/archived jobs for "active" list
      if (status.includes('complete') || status.includes('archived')) continue;

      workOrders.push({
        woNumber: woNumber,
        jobDescription: row[colIndex.jobDescription] || '',
        customerName: row[colIndex.customerName] || '',
        jobType: row[colIndex.jobType] || '',
        jobStatus: row[colIndex.jobStatus] || '',
        jobAddress: row[colIndex.jobAddress] || '',
        salesman: row[colIndex.salesman] || '',
        vehicle: row[colIndex.vehicle] || '',
        trailer: row[colIndex.trailer] || ''
      });
    }

    Performance.end('getActiveWorkOrders');
    ...
    Performance.end('getActiveWorkOrders');
    return ErrorHandler.createErrorResponse(error, 'getActiveWorkOrders');
  }
}

/**
 * Add a new work order to the WorkOrders sheet
 * @param {Object} data - Work order data
 * @returns {Object} Success/failure response
 */
function addWorkOrder(data) {
  Performance.start('addWorkOrder');
  ...
  try {
    ...
    if (!woSheet) {
      return {
        success: false,
        message: 'WorkOrders sheet not found'
      };
    }
    ...
    for (let i = 1; i < existingData.length; i++) {
      if (String(existingData[i][0]) === String(data.woNumber)) {
        return {
          success: false,
          message: `Work order #${data.woNumber} already exists`
        };
      }
    }

    // Build the new row with columns: woNumber, Job Description, Customer Name, Job Type, Job Status, Job Address, Salesman, Vehicle, Trailer, Notes
    const newRow = [
      data.woNumber,
      data.jobDescription || '',
      data.customerName || '',
    ...
    Performance.end('addWorkOrder');

    return {
      success: true,
      message: `Work order #${data.woNumber} added successfully`,
      workOrder: {
    ...
    Performance.end('addWorkOrder');
    return ErrorHandler.createErrorResponse(error, 'addWorkOrder');
  }
}

/**
 * Get all data needed for the TV Shop Dashboard
 * Combines: Active Work Orders, Vehicles, Progress data
 * @returns {Object} Complete TV dashboard data
 */
function getTVDashboardData() {
  Performance.start('getTVDashboardData');
  ...
  try {
    ...
    // Get work orders with salesman and location
    try {
      const woResult = getActiveWorkOrders();
      if (woResult.success) {
        dashboardData.workOrders = woResult.workOrders;
      }
    } catch (e) {
      Logger.log('Error getting work orders for TV dashboard: ' + e.toString());
    }

    // Get vehicle status from Fleet sheet
    try {
      const truckSS = SpreadsheetApp.openById(CONFIG.TRUCK_SHEET_ID);
      const truckSheet = truckSS.getSheetByName(CONFIG.TRUCK_SHEET_NAME);
      const truckData = truckSheet.getDataRange().getValues();
      ...
      for (let i = 1; i < truckData.length; i++) {
        const truckName = truckData[i][0];
        const model = truckData[i][1];
        const year = truckData[i][2];
        const licensePlate = truckData[i][3];
        const status = String(truckData[i][4] || '').toLowerCase();
        const notes = truckData[i][7] || '';
        ...
        const vehicleInfo = {
          ...
          status: status,
          notes: notes,
          assignedJob: null // Will be populated from work orders
        };

        // Check if vehicle is assigned to a job
        const assignedJob = dashboardData.workOrders.find(wo =>
          wo.vehicle && wo.vehicle.toLowerCase().includes(truckName.toLowerCase())
        );
        if (assignedJob) {
          vehicleInfo.assignedJob = {
            woNumber: assignedJob.woNumber,
            jobDescription: assignedJob.jobDescription,
            jobAddress: assignedJob.jobAddress
          };
        }

        if (status.includes('active')) {
          dashboardData.vehicles.active.push(vehicleInfo);
        } else {
          dashboardData.vehicles.inactive.push(vehicleInfo);
        }
      }
    } catch (e) {
      Logger.log('Error getting fleet for TV dashboard: ' + e.toString());
    }

    // Get progress data for work orders
    try {
      const woSS = SpreadsheetApp.openById(CONFIG.ACTIVE_WORK_ORDERS_SHEET_ID);
      const progressSheet = woSS.getSheetByName(CONFIG.PROGRESS_SHEET_NAME);

      if (progressSheet) {
        const progressData = progressSheet.getDataRange().getValues();

        for (let i = 1; i < progressData.length; i++) {
          const woNumber = String(progressData[i][0]);
          if (!dashboardData.progress[woNumber]) {
            dashboardData.progress[woNumber] = { items: [], totalCompleted: 0 };
          }

          const status = progressData[i][3] || 'not-started';
          if (status === 'completed') {
            dashboardData.progress[woNumber].totalCompleted++;
          }

          dashboardData.progress[woNumber].items.push({
            lineIndex: progressData[i][1],
            quantityCompleted: progressData[i][2],
            status: status,
            lastUpdated: progressData[i][5]
          });
        }
      }
    } catch (e) {
      Logger.log('Error getting progress for TV dashboard: ' + e.toString());
    }

    Performance.end('getTVDashboardData');
    ...
    Performance.end('getTVDashboardData');
    return ErrorHandler.createErrorResponse(error, 'getTVDashboardData');
  }
}
